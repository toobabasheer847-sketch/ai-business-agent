...
describe('Master Agent regex router', () => {
  ...
  it('routes create/list/complete task phrasing to task_agent', async () => {
    ...
    await expect(
      router(ALL_AGENTS, messageContext('How many tasks are overdue?')),
    ).resolves.toBe('task_agent');
    await expect(
      router(ALL_AGENTS, messageContext('Show my task statistics')),
    ).resolves.toBe('task_agent');
    await expect(
      router(ALL_AGENTS, messageContext('Give me a report of my tasks')),
    ).resolves.toBe('task_agent');
    await expect(
      router(ALL_AGENTS, messageContext('Show my task trends this month')),
    ).resolves.toBe('task_agent');
  });
